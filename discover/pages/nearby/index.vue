...
const [nearby, popular, favorite, newToday] = await Promise.all([
  useFetch('/discover/nearbyDeals', {
    transform: (apiResponse) => dealCardsWithMarkerMapper(apiResponse, categoriesById.value),
  }),
  useFetch('/discover/nearbyPopularDeals?limit=15', {
    transform: (apiResponse) => dealCardsWithMarkerMapper(apiResponse, categoriesById.value),
  }),
  useFetch('/discover/nearbyFavoriteDeals', {
    transform: (apiResponse) => dealCardsWithMarkerMapper(apiResponse, categoriesById.value),
  }),
  useFetch('/discover/nearbyNewTodayDeals', {
    transform: (apiResponse) => dealCardsWithMarkerMapper(apiResponse, categoriesById.value),
  }),
]);
...
const availableCount = computed(
  () =>
    filteredAllDeals.value.length +
    filteredNewTodayDeals.value.length +
    filteredPopularDeals.value.length,
);

const { data: singleInspirationInformation } = await useFetch('/discover/single_inspiration', {
  transform: singleInformationItemMapper,
});

const { data: multipleInspirationInformation } = await useFetch('/discover/multiple_inspiration', {
  transform: (list: InformationBlockDTO[]) => multipleInformationMapper(list.slice(1)),
});
...
<template>
  ...
  >
    ...
    <InformationBlockSingleInspiration
      ...
      v-bind="singleInspirationInformation"
    />
    ...
    <InformationBlockMultipleInspiration
      ...
      :blocks="multipleInspirationInformation"
