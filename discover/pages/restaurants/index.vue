...
const { data: categoryData } = await useFetch('/discover/restaurantCategories', {
...
const [nearby, popular, favorite, newToday] = await Promise.all([
  useFetch('/discover/restaurantDeals', {
    transform: (apiResponse) => dealCardsWithMarkerMapper(apiResponse, categoriesById.value),
  }),
  useFetch('/discover/restaurantPopularDeals?limit=15', {
    transform: (apiResponse) => dealCardsWithMarkerMapper(apiResponse, categoriesById.value),
  }),
  useFetch('/discover/restaurantFavoriteDeals', {
    transform: (apiResponse) => dealCardsWithMarkerMapper(apiResponse, categoriesById.value),
  }),
  useFetch('/discover/restaurantNewTodayDeals', {
    transform: (apiResponse) => dealCardsWithMarkerMapper(apiResponse, categoriesById.value),
  }),
]);
...
const { data: restaurantInformation } = await useFetch('/discover/restaurant', {
  transform: restaurantItemMapper,
});
...
<template>
  <HeaderSection
    ...
    :title="title.HeaderSectionTitleRestaurant"
  ...
  >
    ...
    >
      <template #card="{ deal, toggleFavorite }">
        <DealCardRestaurant
          v-bind="deal"
